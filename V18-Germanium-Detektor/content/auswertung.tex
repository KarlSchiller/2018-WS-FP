...
Raumwinkel anhand Formel \eqref{eqn:Raumwinkel} berechnet. Dabei ergibt mit
$a = \SI{22.3(10)}{\milli\meter}$ und $r = \SI{22.5(1)}{\milli\meter}$ sich:
\begin{align*}
  \frac{\Omega}{4\pi} = \num{0.01558(34)}
\end{align*}
...
\begin{align*}
    A_\text{Messtag} = A_0 \exp{\left(-\frac{\ln{2} \cdot\Delta t}{t_{1/2}}\right)}=\SI{1633(24)}{\becquerel}
\end{align*}
...
\begin{figure}
  ...
  \caption{Vom Detektor aufgenommenes Spektrum der Cäsium-Quelle, dargestellt
...
\begin{table}[htb]
	\centering
  \caption{Charakteristische Peaks des Cs-Strahlers ahnand der Bin-Indizes und der Energie $E$.}
  \label{tab:Cs_char}
  \begin{tabular}{c
    S[table-format=4.0]
    S[table-format=3.2]}
...
untersuchten Compton-Kontinuums und des Vollenergiepeaks wird durch ein
Aufintegrieren dieser Formel bestimmt auf:
\begin{align*}
  Z_\text{Compton} &= \num{10580(9)} \\
  Z_\text{Voll} &= \num{19697.88}
\end{align*}
Zu erkennen ist ganz klar, dass der Vollenergiepeak wesentlich höher liegt als
das Compton-Kontinuum. Dies entsteht dadurch trotz der geringeren
Absorptinoswahrscheinlichkeit dadurch, dass der Vollenergiepeak nicht durch den
...
Hier sollte anhand eines Spektrums zwischen Barium(\ce{^{133}Ba}) und Antimon(\ce{^{125}Sb})
unterschieden werden. Hierfür wurde das in Abbildung \ref{plt:bar} zu sehende
...
charakteristisch für barium sind, sind allerdings nicht gefunden worden. Jene
Peaks jedoch, die klar zu erkennen sind, aber nicht zugeordnet wurden, sind auf
...
\ref{plt:aktivitaet_ba} wurde dann durch bildung eines Mittelwertes die
Aktivität bestimmt auf:
\begin{align*}
...
Hier wurde das in Abbildung \ref{plt:unbekannt} dazu genutzt die Peaks zu
herauszusuchen. Anschließend wurden deren Höhe und Energie bestimmt. Die daraus
...
Dort gibt es nun zwei Nuklide, die hier vorkommen könnten, \ce{^{214}Bi} und
...
entsprächen die gefundenen Emissionslinien denLinien bei \SI{1210}{\kilo\electronvolt}
